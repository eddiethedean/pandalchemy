...
def test_multi_table_push_partial_failure_rollback(tmp_path):
    ...
    orders = pd.DataFrame({"id": [1], "user_id": [1], "total": [100.0]})
...
def test_incremental_push_pattern(tmp_path):
    ...
    # Create table
    data = pd.DataFrame({"id": [1], "value": [0]})
...
def test_complex_five_table_failure_rollback(tmp_path):
    ...
    for i in range(1, 6):
        data = pd.DataFrame(
            {"id": [1, 2], "value": [i * 10, i * 20], "name": [f"Table{i}_A", f"Table{i}_B"]}
        )
...
def test_checkpoint_and_resume_pattern(tmp_path):
    ...
    jobs = pd.DataFrame({"id": range(1, 101), "status": ["pending"] * 100, "result": [None] * 100})
    ...
    for batch_start in range(1, 101, batch_size):
        ...
        for job_id in range(batch_start, batch_end):
            # Simulate processing
            db["jobs"].update_row(
                job_id, {"status": "completed", "result": f"Result for job {job_id}"}
            )
...
def test_handling_constraint_violation_during_push(tmp_path):
    ...
    users = pd.DataFrame(
        {"id": [1, 2, 3], "email": ["alice@test.com", "bob@test.com", "charlie@test.com"]}
    )
    db.create_table("users", users, primary_key="id")
...
def test_data_validation_all_types_before_push(tmp_path):
    ...
    memberships = pd.DataFrame(
        {"user_id": [1, 2], "org_id": ["org1", "org2"], "role": ["admin", "user"]}
    )
