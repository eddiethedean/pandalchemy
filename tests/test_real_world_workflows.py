...
def ecommerce_db(tmp_path):
    ...
    order_items = pd.DataFrame(
        {"order_id": [2, 2], "product_id": [101, 102], "quantity": [2, 1], "price": [19.99, 29.99]}
    )
...
def test_ecommerce_order_processing(ecommerce_db, tmp_path):
    ...
    db["orders"].add_row({"id": 3, "customer_id": 1, "status": "pending", "total": 0.0})
...
def test_ecommerce_order_cancellation(ecommerce_db):
    ...
    for _, item in db["order_items"][
        db["order_items"].index.get_level_values("order_id") == 2
    ].iterrows():
...
def test_user_registration_workflow(tmp_path):
    ...
    db["users"].add_row(
        {"username": "charlie", "email": "charlie@test.com", "active": True}, auto_increment=True
    )
...
def test_user_update_workflow(tmp_path):
    ...
    # Setup
    users = pd.DataFrame(
        {"id": [1], "username": ["alice"], "email": ["alice@old.com"], "active": [True]}
    )
    ...
    profiles = pd.DataFrame({"user_id": [1], "full_name": ["Alice"], "bio": ["Developer"]})
    ...
    permissions = pd.DataFrame({"user_id": [1], "resource_id": ["posts"], "can_write": [False]})
    db.create_table("user_permissions", permissions, primary_key=["user_id", "resource_id"])
    ...
    db["user_profiles"].update_row(1, {"full_name": "Alice Smith", "bio": "Senior Developer"})
...
def test_cms_article_publishing_workflow(tmp_path):
    ...
    assert pd.isna(reviewed_by), (
        "Schema changes and immediate updates in same transaction is a known limitation"
    )
...
def test_cms_article_versioning(tmp_path):
    ...
    articles = pd.DataFrame(
        {"id": [1], "title": ["Original Title"], "content": ["Original content"], "version": [1]}
    )
    db.create_table("articles", articles, primary_key="id")
    ...
    db["articles"].update_row(
        1, {"title": "Updated Title", "content": "Updated content", "version": 2}
    )
...
def test_financial_transfer_workflow(tmp_path):
    ...
    total_after = db["accounts"].get_row(1)["balance"] + db["accounts"].get_row(2)["balance"]
...
def test_financial_insufficient_funds_rollback(tmp_path):
    ...
    accounts = pd.DataFrame(
        {"id": [1, 2], "name": ["Checking", "Savings"], "balance": [50.0, 1000.0]}
    )
    db.create_table("accounts", accounts, primary_key="id")

    transactions = pd.DataFrame({"id": [], "from_account": [], "to_account": [], "amount": []})
    db.create_table("transactions", transactions, primary_key="id")
...
def test_multi_currency_financial_system(tmp_path):
    ...
    db.create_table("exchange_rates", exchange_rates, primary_key=["from_currency", "to_currency"])
...
def test_user_deactivation_cascade(tmp_path):
    ...
    users = pd.DataFrame({"id": [1, 2], "username": ["alice", "bob"], "active": [True, True]})
    db.create_table("users", users, primary_key="id")
