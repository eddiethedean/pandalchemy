...
def test_type_change_preserves_data_integrity(tmp_path):
    ...
    df = pd.DataFrame(
        {"id": [1, 2, 3], "product": ["Coffee", "Tea", "Juice"], "price_cents": [350, 275, 425]}
    )
...
def test_type_change_date_string_to_datetime_equivalent(tmp_path):
    ...
    df = pd.DataFrame({"id": [1, 2, 3], "event_date": ["2024-01-15", "2024-02-20", "2024-03-10"]})
...
def test_convert_percentage_strings_to_float(tmp_path):
    ...
    df = pd.DataFrame(
        {"id": [1, 2, 3], "question": ["Q1", "Q2", "Q3"], "approval_rate": ["75%", "82%", "68%"]}
    )
